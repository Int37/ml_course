...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = np.random.randn(scale =weight_scale,layer_input_dim, hd)
            self.params['b%d'%(i+1)] = np.zeros(scale=weught_scale ,hd)
            if self.use_batchnorm:
                self.params['gamma%d'%(i+1)] = np.ones(hd)
                self.params['beta%d'%(i+1)] = np.zeros(hd)
            layer_input_dim = hd
        self.params['W%d'%(self.num_layers)] =  np.random.randn(scale =weight_scale ,layer_input_dim, num_classes)
        self.params['b%d'%(self.num_layers)] = np.zeros(scale=weight_scale ,num_classes)"""
